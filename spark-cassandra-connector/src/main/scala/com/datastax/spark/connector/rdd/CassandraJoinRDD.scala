...
class CassandraJoinRDD[Left, Right] private[connector](prev: RDD[Left],
  ...
  override protected def copy(columnNames: ColumnSelector = columnNames,
    ...
    new CassandraJoinRDD[Left, Right](prev, keyspaceName, tableName, connector, columnNames, joinColumns, where, readConf).asInstanceOf[this.type]
  ...
  protected def checkValidJoin(columns: Seq[NamedColumnRef]): Seq[NamedColumnRef] = {
    ...
    columns.foreach(checkSingleColumn)
    columns
  }
  ...
  override def toEmptyCassandraRDD(): EmptyCassandraRDD[(Left, Right)] = new EmptyCassandraRDD[(Left, Right)](prev.sparkContext, keyspaceName, tableName, columnNames, where, readConf)
}